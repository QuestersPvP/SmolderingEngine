...
*
*/
void Texture2D::loadFromFile(std::string filename, VkFormat format, VkPhysicalDevice* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout, bool forceLinear)
{
	...
	VkImageLayout						imageLayout)
{
	...
	{
		std::cout << "Error creating buffer";
	}
	...
	memAllocInfo.allocationSize = memReqs.size;
	// Get memory type index for a host visible buffer
	memAllocInfo.memoryTypeIndex = GetMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, memoryProperties);
	...
	{
		std::cout << "Error ending command buffer";
	}
	...
	if (vkCreateSampler(*logicalDevice, &samplerCreateInfo, nullptr, &sampler) != VK_SUCCESS)
	{
		std::cout << "Error creating sampler";
	}
	...
	viewCreateInfo.image = image;
	if (vkCreateImageView(*logicalDevice, &viewCreateInfo, nullptr, &view) != VK_SUCCESS)
	{
		std::cout << "Error creating image view";
	}

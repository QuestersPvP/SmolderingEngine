...
namespace SE_Renderer
{
    ...
    uint32_t GetMemoryType(uint32_t _typeBits, VkMemoryPropertyFlags _properties, VkPhysicalDeviceMemoryProperties& _memoryProperties, VkBool32* _memTypeFound = nullptr);
	VkShaderModule LoadShaderModule(const char* _fileName, VkDevice _logicalDevice);

	// TODO: MAKE ALL INLINE THINGS INTO THEIR OWN FILE
	VkPipelineShaderStageCreateInfo LoadShader(std::string fileName, VkShaderStageFlagBits stage, VkDevice _logicalDevice);

	inline VkApplicationInfo GenerateApplicationInfo()
	{

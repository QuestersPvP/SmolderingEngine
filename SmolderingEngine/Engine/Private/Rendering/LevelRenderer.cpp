...
// TODO: REMOVE GAME STUFF FROM HERE
...
LevelRenderer::LevelRenderer(const VulkanResources* _resources, class Game* _game)
	: vulkanResources(_resources), seGame(_game)
{
	try
	{
...
void LevelRenderer::RecordToCommandBuffer(VkCommandBuffer _commandBuffer, uint32_t _imageIndex)
{
	...
	for (size_t i = 0; i < seGame->gameObjects.size(); i++)
	{
		...
		vkCmdPushConstants(_commandBuffer, graphicsPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(Model), &seGame->gameObjects[i]->GetModel());

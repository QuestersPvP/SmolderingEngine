...
InputManager::InputManager(const std::string& _windowName, int _width, int _height)
{
    glfwInit();
    ...
    lastMouseX = _width / 2.f;
    lastMouseY = _height / 2.f;
...
{
    InputManager* inputManager = static_cast<InputManager*>(glfwGetWindowUserPointer(_window));
   
    if (inputManager) 
    {
        if (_action == GLFW_PRESS)
        {
            inputManager->keyStates[_key] = true;
        }
        else if (_action == GLFW_RELEASE)
        {
            inputManager->keyStates[_key] = false;
...
void InputManager::MouseCallback(GLFWwindow* _window, double _xPos, double _yPos)
{
    ...
    if (inputManager)
    {
        inputManager->currentMouseX = _xPos;
        inputManager->currentMouseY = _yPos;
    }
}
